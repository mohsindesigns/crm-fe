...
// routes. Same "pay via CRM" UX; the invoice itself has no bearing on company
// revenue.
...
export default function PublicPersonalInvoicePage() {
  ...
  const apiBase = '/api';
  const pdfUrl = `${apiBase}/public/personal-invoices/${token}/pdf`;

  function load() {
    fetch(`${apiBase}/public/personal-invoices/${token}`)
      .then(async (r) => {
  ...
  async function pay() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${apiBase}/public/personal-invoices/${token}/pay`, {
        method: 'POST',
